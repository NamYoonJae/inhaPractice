...
void cLavaGolem::CreateShadow()
{
	...
	D3DXMATRIXA16 oldWorldMat = m_pSkinnedMesh->m_matWorldTM;
	m_pSkinnedMesh->m_matWorldTM = _mTM;

	m_pSkinnedMesh->UpdateMatrix();
	g_pD3DDevice->SetTransform(D3DTS_WORLD, &_tempMat);
	m_pSkinnedMesh->Render();
	m_pSkinnedMesh->m_matWorldTM = oldWorldMat;
	m_pSkinnedMesh->UpdateMatrix();
